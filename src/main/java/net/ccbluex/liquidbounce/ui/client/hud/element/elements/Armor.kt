    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        if (mc.playerController.isNotCreative) {
            ...
            for (index in 3 downTo 0) {
                val colorall = when {
                    rainbow -> 0
                    colorMode.equals("Astolfo", ignoreCase = true) -> RenderUtils.Astolfo(index * speed.get(), saturationValue.get(), brightnessValue.get())
                    else -> color
                }
                ...
                Fonts.font28.drawStringWithShadow(((stack.maxDamage - stack.itemDamage)).toString(),x.toFloat() + 4f,20f,colorall)
